...
Claude Code Skill，自动抓取 65+ 英文 AI 信息源（含 [follow-builders](https://github.com/zarazhangrui/follow-builders) 25 位一线 AI 构建者），多维度评分筛选，AI 选题分三梯队，生成 9 张卡片（封面 1080×1920px + 8 张内页）用于小红书分享。
...
3. **Curate** — Claude 从 100+ 候选中精选 24 条，分三梯队
4. **Render** — JSON 注入 HTML 模板，生成 9 个页面
5. **Screenshot** — Playwright 截图，封面 1080×1920，内页 1080×1920 @2x
...
| P2–P5 | `feature.html` | 第一梯队，每页一条 + 150 字摘要 |
| P6–P7 | `half-page.html` | 第二梯队，每页两条 + 80 字摘要 |
| P8–P9 | `briefs.html` | 快讯，每页 8 条 2×4 网格 |

## 设计规范

- **封面（P0）** — `1080×1920px`，内容在中间 3:4 安全区（上 280px / 下 240px padding），确保小红书缩略图不裁切内容
- **内页** — `1080×1920px`，@2x 输出 `2160×3840px`
