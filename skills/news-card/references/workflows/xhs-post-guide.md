...
2. **英文替代**（次选）：仅在中文绕不开时使用，且必须正确大写（Data Center 不是 data center）
3. **Unicode 数学字体**（末选）：英文也敏感时用 `𝙈𝙖𝙩𝙝 𝙁𝙤𝙣𝙩`

**⚠️ 优先用中文同义词**——生硬插入英文（如 cybersecurity、public safety）会破坏编辑调性。
...
| 黑客 | 入侵者 / 恶意行为者 | Hacker |
| 数据 | 信息 / 资料 | Data |
| 数据中心 | 算力中心 | Data Center |
| 成本 | 花费 / 开支 | Cost |
| 漏洞 | 缺陷 / 弱点 | Bug |
| 泄露 | 流出 / 外传 | Leak |

> 完整替换库 → `references/config/sensitive-word-dict.md`
